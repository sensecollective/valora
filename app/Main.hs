...
main :: IO ()
main =
  writeImageToBMP "out.bmp" $ rasterLayer $ applyLayer bg $ fromMask (frameSize, frameSize) redRects
  where
    redRects =
      H.unionsWith applyPixel $
      map (\rs -> waterColor (map (\r -> ((1, 0, 0, 1), r)) rs) applyPixel) rects
