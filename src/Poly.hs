...
import Coords
       (Point(..), Subrange(..), extractSubrange, pixelCoords,
...
centroid :: Poly -> Point
centroid Poly {vertices} = Point {x = (right + left) / 2, y = (top + bottom) / 2}
  where
    left = V.minimum xs
    right = V.maximum xs
    top = V.maximum ys
...
inExtent Extent {bottomLeft = Point {x = lowX, y = lowY}, width, height} Point {x, y} =
